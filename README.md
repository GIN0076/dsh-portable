...
```
DSH-Portable/
├── addons/          # Out-of-tree extension layer (not part of official src)
...
│   ├── market/         # M3 plugin market CLI (search/info/install via M2 gate)
│   ├── translate/      # M3 Chinese translation layer
│   ├── workshop/       # M4 plugin workshop (generate → review → install)
│   └── update-ui/      # M5 "Update Check" settings card (dsh-update-check plugin)
├── shell/           # Electron desktop shell (main.js + package.json)
├── installer/       # Inno Setup wizard (DSH-Portable.iss)
├── scripts/         # stop/clean PowerShell scripts
...
- This project's own code (`addons/`, `shell/`, `installer/`, `scripts/`, `build/`, docs): **MIT** — see [LICENSE](LICENSE).
