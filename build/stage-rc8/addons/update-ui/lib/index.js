/**
 * dsh-update-check host entry: mounts a loopback-only HTTP route that runs
 * the DSH-Portable update engine (addons/update-engine/update-dsh.ps1) in
 * check mode and returns the result as JSON for the settings card.
 */
      ...
      const dispose = host.webServer.register({
        ...
        handler: async (request, response) => {
          ...
          if (!isTrustedOrigin(request.headers.origin)) {
            ...
            response.end(JSON.stringify({ error: 'update check is limited to loopback requests' }))
            return
          }
          try {
            ...
            response.end(JSON.stringify({ status: 'error', detail: String(err && err.message ? err.message : err) }))
          }
        },
      })
      return dispose
    }, 'dsh-update-check: http route')
  })
}
