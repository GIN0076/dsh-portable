/**
 * Browser client bundle for the dsh-update-check plugin, mirroring the
 * DeepSeek Harness client preset (packages/client/tsdown.client.ts) and the
 * dsh-market plugin's external-package recipe: a closure-factory artifact
 * that calls window.__ModuleLoader__.load({ id, factory }) and resolves
 * externals through the injected require (loader module table).
 */
