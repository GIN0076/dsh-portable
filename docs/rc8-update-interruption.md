...
> **归档记录 / Archival note（2026-08-28）**：本文档记录的是 RC7 → RC8 时代（2026-08-20）的一次性事件。当前发行版（`0.1.2-alpha.1` 及以后）**不再依赖官方自动更新器**——已默认禁用 GUI 自动更新入口（托盘 / Web 卡片），高级用户可手动调用 `packages/update-engine/update-dsh.ps1`。本文件保留作历史参考。
...
>
> **致用户**：由于官方更新器在当前 Windows 环境下无法可靠完成自动升级，本项目现已改为**直接封装发布 RC8 安装包**（手工构建、已验证），不再依赖官方更新器自动升级。因此给您带来的不便，敬请谅解。
