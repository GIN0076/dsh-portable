...
- **Electron shell** (`apps/desktop-shell/`) — window + tray experience; the DSH web service runs on `127.0.0.1:3080` as a child process.
- **Extension layer** (`packages/`) — update engine, audit ledger, privacy center, plugin security review, plugin market, translation layer, plugin workshop, and the "Update Check" settings card.
...
```
DSH-Portable/
├── packages/                 # out-of-tree extension layer
...
- `停止DSH-Portable.cmd` → `scripts/windows/stop-dsh.ps1`: kill by port 3080 listener **and** by command-line match on the app root (electron.exe/node.exe), `taskkill /T /F`, no residue.
- `清理DSH-Portable.cmd` → `scripts/windows/clean-dsh.ps1`: stop → remove desktop/Start-Menu shortcuts → optionally delete the data directory (`-DeleteData` / `-KeepData` / interactive) → silent uninstall via `unins*.exe`.
...
### M1 — Update engine (`packages/update-engine/update-dsh.ps1`)
...
### M1 — Audit ledger (`packages/audit/dsh-audit.ps1`)
...
### M1 — Privacy center (`packages/privacy/dsh-privacy.ps1`)
...
### M2 — Plugin security review (`packages/plugin-review/`)
...
### M3 — Plugin market (`packages/plugin-market/dsh-market.ps1`)
...
### M3 — Translation layer (`packages/translation/dsh-translate.ps1`)
...
### M4 — Plugin workshop (`packages/plugin-workshop/dsh-workshop.ps1`)
...
### M5 — Update Check UI (`packages/update-ui/`, plugin `dsh-update-check`)
...
| Decision | Rationale |
|---|---|
| Zero-modification `src/` + out-of-tree `packages/` | Upstream updates stay clean; custom layer is versionable and re-appliable |
