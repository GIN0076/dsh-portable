...
├── scripts/windows/       # Windows cmd wrappers
├── packages/              # this repo's extension packages/
├── scripts/windows/       # Windows cmd wrappers
├── README.md
...
`version-manifest.json` / `upstream-lock.json` are produced by `packages/update-engine/update-dsh.ps1` (`-Mode apply`) or written manually; keep them truthful — the update engine verifies against them.
