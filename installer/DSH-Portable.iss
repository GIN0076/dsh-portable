...
Source: "{#SourceRoot}\shell\*"; DestDir: "{app}\shell"; Flags: ignoreversion recursesubdirs createallsubdirs; Excludes: "launcher-config.json,install-log.txt"
Source: "{#SourceRoot}\addons\*"; DestDir: "{app}\addons"; Flags: ignoreversion recursesubdirs createallsubdirs
Source: "{#SourceRoot}\scripts\*"; DestDir: "{app}\scripts"; Flags: ignoreversion recursesubdirs createallsubdirs
Source: "{#SourceRoot}\停止DSH-Portable.cmd"; DestDir: "{app}"; Flags: ignoreversion
Source: "{#SourceRoot}\清理DSH-Portable.cmd"; DestDir: "{app}"; Flags: ignoreversion
...
[Run]
Filename: "{app}\{#ElectronExe}"; Parameters: """{app}\shell"""; WorkingDir: "{app}"; Flags: nowait postinstall skipifsilent

[UninstallDelete]
Type: files; Name: "{app}\shell\launcher-config.json"
...
begin
  ...
  begin
    ...
    RegWriteStringValue(HKCU, RunKey, '{#MyAppName}',
      '"' + AppDir + '\{#ElectronExe}" "' + AppDir + '\shell"');
...
begin
  ...
  ConfigPath := ExpandConstant('{app}\shell\launcher-config.json');
...
begin
  ...
  begin
    ...
    DeleteFile(AppDir + '\shell\launcher-config.json');
